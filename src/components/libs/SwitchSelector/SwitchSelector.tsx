...
export default class SwitchSelector extends Component {
  ...
  constructor(props: any) {
    ...
    this.animatedValue = new Animated.Value(initial ? (I18nManager.isRTL ? -(initial / options.length) : initial / options.length) : 0);
  }

  componentDidUpdate(prevProps: any) {
    const { value, disableValueChangeOnPress } = this.props as any;
  ...
  getBgColor() {
    const { selected } = this.state as any;
    const { options, buttonColor } = this.props as any;
    if (selected === -1) {
      return 'transparent';
    }
  ...
  responderEnd = (evt: any, gestureState: any) => {
    const { disabled, options } = this.props as any;
    const { selected } = this.state as any;
  ...
  shouldSetResponder = (evt: any, gestureState: any) =>
    evt.nativeEvent.touches.length === 1 && !(Math.abs(gestureState.dx) < 5 && Math.abs(gestureState.dy) < 5);

  animate = (value: any, last: any) => {
    const { animationDuration } = this.props as any;
  ...
  toggleItem = (index: any, callOnPress = true) => {
    const { selected } = this.state as any;
    const { options, returnObject, onPress } = this.props as any;
  ...
  render() {
    ...
    const { selected, sliderWidth } = this.state as any;

    const optionsMap = options.map((element: any, index: any) => {
    ...
    return (
      <View style={[{ flexDirection: 'row' }, style]} accessibilityLabel={accessibilityLabel} testID={testID}>
...
(SwitchSelector.defaultProps as any) = {
  style: {},
  textStyle: {},
  selectedTextStyle: {},
  textContainerStyle: {},
...
(SwitchSelector.propTypes as any) = {
